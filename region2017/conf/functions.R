...
CS <- function(layers){
	...
	########################################################
		...
		###########################
		total_extent <- extent%>%group_by(habitat, scenario_year)%>%summarize(total_extent = sum(km2, na.rm = T))%>%ungroup()
		...
		cs_all <- merge(cs_all, total_extent, by = c("habitat", "scenario_year"))
		...
		#cs_all$status_score <- (((cs_all$health/cs_all$reference_point)*(cs_all$km2))/cs_all$total_extent)*100
...
CP <- function(layers){
	...
	########################################################
		...
		cp_all <- merge(cp_all, total_extent, by = c("habitat", "scenario_year"))
...
TR <- function(layers){
	...
	ittc <- SelectData2(ittc_lyrs)
		...
		#change colnames
		#################
		names(ittc) <- c("layer_name", "scenario_year", "data_year", "rgn_id", "values")
	
	#length stay international tourist
	##################################
	lstay_inter_lyrs <- c("tr_lengthstay_international")
	lstay_inter <- SelectData2(lstay_inter_lyrs)
	
		#change colnames
		#################
		names(lstay_inter) <- c("layer_name", "scenario_year", "data_year", "rgn_id", "values")
	
		
	#length stay domestic tourist
	#############################
	lstay_dom_lyrs <- c("tr_lengthstay_domestic")
	lstay_dom <- SelectData2(lstay_dom_lyrs)
	
		#change colnames
		#################
		names(lstay_dom) <- c("layer_name", "scenario_year", "data_year", "rgn_id", "values")
	
		
	#occupancy rate
	###############
	occupancy_lyrs <- c("tr_occupancy_rate")
	occupancy <- SelectData2(occupancy_lyrs)
	
		#change colnames
		#################
		names(occupancy) <- c("layer_name", "scenario_year", "data_year", "rgn_id", "values")
	
	
	
	
	tr_all <- rbind(ittc, lstay_inter, lstay_dom, occupancy)
	
	#determine reference point for each layer
	#########################################
	ref <- tr_all %>% group_by(layer_name, rgn_id)%>%summarize(ref_values = max(values, na.rm = T))%>%ungroup()
	
		
		
	
	#merge layers with reference points
	###############################	
	tr_all <- merge(tr_all, ref, by = c("layer_name", "rgn_id"))
	
			
	#calculate status score for each year-region combining four layers
	#############################################################
	tr_all$status_score <- ifelse(is.na(tr_all$values), NA, (tr_all$values/tr_all$ref_values))
	print(tr_all)
	
		
		am <- NULL
		for (aa in unique(tr_all$scenario_year)){
		for (ab in unique(tr_all$rgn_id)){
			
			al <- which(tr_all$scenario_year == aa & tr_all$rgn_id == ab)
		...
		if (length(ald > 0)){
			...
			if (nrow(ald) > 3){
			 ...
			 aldk <- data.frame(rgn_id = ab, year = aa, status_score = prod(ald$status_score))
			
			am <- rbind(am, aldk)	
				
			}else{
				
			print("layers are not complete")
...
HAB <- function(layers){
	...
	print(reference_point)
		...
		############################################################
		 ...
		 write_csv(weights, "~/github/bali/region2017/layers/element_wts_hab__pres_abs.csv")
...
FP= function(layers, scores){
	...
	w_fis <- dat_weight%>%group_by(rgn_id, scenario_year)%>%summarize(w_FIS = sum(ton.x, na.rm = T)/(sum(ton.x, na.rm = T)+sum(ton.y, na.rm = T)))%>%ungroup()
		 
	w_mar <- dat_weight%>%group_by(rgn_id, scenario_year)%>%summarize(w_MAR = sum(ton.y, na.rm = T)/(sum(ton.x, na.rm = T)+sum(ton.y, na.rm = T)))%>%ungroup()
	
	#merge fis weight and mar weight
	w <- merge(w_fis, w_mar, by = c("rgn_id", "scenario_year"))
	
		
		weight <- w%>%rename(rgn_id = rgn_id, year = scenario_year, w_FIS = w_FIS, w_MAR = w_MAR)
	
	
		weight <- weight%>%select(rgn_id, year, w_FIS)%>%rename(rgn_id = rgn_id, year = year, w_fis = w_FIS)
	
		write_csv(weight, "~/github/bali/region2017/layers/fp_wildcaught_weight_bali2016.csv")
	
	
		
	
	w <- w%>%rename(region_id = rgn_id, year = scenario_year, w_FIS = w_FIS, w_MAR = w_MAR)
	
	
		w <- w%>%group_by(region_id)%>%summarize(w_FIS = mean(w_FIS, na.rm = T), w_MAR = mean(w_MAR, na.rm = T))%>%ungroup()
...
NP <- function(layers, scores){
	
	#idea of status score: seaweed production and non-star hotel occupancy rate
	#many of seaweed farmer switches its profession to work in tourism field
	###########################################################################
	
	#seaweed production layer
	#########################
	seaweed_lyrs <- c("np_seaweed_production")
	seaweed <- SelectData2(seaweed_lyrs)
	...
	#combine layers
	###############
	np_all <- rbind(seaweed, occupancy)
	
	#determine reference point for each layer
	#########################################
	ref <- np_all %>% group_by(layer_name, rgn_id)%>%summarize(ref_values = max(values, na.rm = T))%>%ungroup()
	...
	#merge layers with reference points
	###############################	
	np_all <- merge(np_all, ref, by = c("layer_name", "rgn_id"))
	
			
	#calculate status score for each year-region combining two layers
	#################################################################
	np_all$status_score <- ifelse(is.na(np_all$values), NA, (np_all$values/np_all$ref_values))
	print(np_all)
	
		
		am <- NULL
		for (aa in unique(np_all$scenario_year)){
		for (ab in unique(np_all$rgn_id)){
			
			al <- which(np_all$scenario_year == aa & np_all$rgn_id == ab)
		...
		if (length(ald > 0)){
			...
			if (nrow(ald) > 1){
				...
				ald <- ald[!is.na(ald$status_score),]
				
				if (nrow(ald) > 1){
					
					aldk_seaweed <- ald[ald$layer_name == "np_seaweed_production",]
					aldk_occupancy <- ald[ald$layer_name == "tr_occupancy_rate",]
						
					aldk <- data.frame(rgn_id = ab, year = aa, status_score = aldk_seaweed$status_score/aldk_occupancy$status_score)
					...
					print("no seaweed data production")
		...
		#status NP based on five latest year observation
		################################################
		am <- am%>%filter(!is.na(status_score))%>%group_by(rgn_id)%>%arrange(year)%>%top_n(5, year)%>%summarize(score = mean(status_score, na.rm = T)*100)%>%ungroup()
		...
		scores <- rbind(status, trend)%>%mutate(goal = "NP")%>%data.frame()
		scores <- scores%>%select(goal, dimension, region_id, score)
...
CW <- function(layers, scores){
	...
	##############
		...
		#read each status layer, calculate mean over each subregion		
		###########################################################
		
		
		status_bod <- layers$data$cw_bod_status
		status_cadmium <- layers$data$cw_cadmium_status
		status_cod <- layers$data$cw_cod_status
		status_coliform <- layers$data$cw_coliform_status
		status_do <- layers$data$cw_do_status
		status_fosfat <- layers$data$cw_fosfat_status
		status_nitrat <- layers$data$cw_nitrat_status
		status_ph <- layers$data$cw_ph_status
		status_water_clarity <- layers$data$cw_water_clarity_status
		
		status_lyrs <- rbind(status_bod, status_cadmium, status_cod, status_coliform, status_do, status_fosfat, status_nitrat, status_ph, status_water_clarity)
		
		status_lyrs <- status_lyrs%>%group_by(rgn_id)%>%summarize(score = mean(score, na.rm = T)*100)%>%ungroup()
		
		status <- status_lyrs%>%mutate(dimension = "status")%>%rename(region_id = rgn_id, score = score, dimension = dimension)%>%select(region_id, score, dimension)
		
			
	#trend layers
	#############
	
		#read each trend layer, calculate mean over each subregion		
		###########################################################
		
		trend_bod <- layers$data$cw_bod_trend
		trend_cadmium <- layers$data$cw_cadmium_trend
		trend_cod <- layers$data$cw_cod_trend
		trend_coliform <- layers$data$cw_coliform_trend
		trend_do <- layers$data$cw_do_trend
		trend_fosfat <- layers$data$cw_fosfat_trend
		trend_nitrat <- layers$data$cw_nitrat_trend
		trend_ph <- layers$data$cw_ph_trend
		trend_water_clarity <- layers$data$cw_water_clarity_trend
		
		
		trend_lyrs <- rbind(trend_bod, trend_cadmium, trend_cod, trend_coliform, trend_do, trend_fosfat, trend_nitrat, trend_ph, trend_water_clarity)
		
				
		trend_lyrs <- trend_lyrs%>%group_by(rgn_id)%>%summarize(score = mean(score, na.rm = T))%>%ungroup()
		
		trend <- trend_lyrs%>%mutate(dimension = "trend")%>%rename(region_id = rgn_id, score = score, dimension = dimension)%>%select(region_id, score, dimension)
	
	
	
	
		scores <- rbind(status, trend)%>%mutate(goal = "CW")%>%data.frame()
		scores <- scores%>%select(goal, dimension, region_id, score)
...
LIV <- function(layers){
	...
	#number variety of marine enterprises
	#####################################
	variety_lyrs <- c("liv_variety_marine_enterprises")
	variety <- SelectData2(variety_lyrs)
	
		#change colnames
		#################
		names(variety) <- c("layer_name", "scenario_year", "data_year", "rgn_id", "values")
		
			
		
	
	#number small-medium marine enterprises
	########################################
	small_medium_lyrs <- c("liv_small_medium_enterprises")
	small_medium <- SelectData2(small_medium_lyrs)
	
		#change colnames
		#################
		names(small_medium) <- c("layer_name", "scenario_year", "data_year", "rgn_id", "values")
		
			
			
	
	liv_all <- rbind(variety, small_medium)
	
	#determine reference point for each layer
	#########################################
	ref <- liv_all %>% group_by(layer_name, rgn_id)%>%summarize(ref_values = max(values, na.rm = T))%>%ungroup()
	...
	#merge layers with reference points
	###############################	
	liv_all <- merge(liv_all, ref, by = c("layer_name", "rgn_id"))
	
	
				
	#calculate status score for each year-region combining two layers
	#################################################################
	liv_all$status_score <- ifelse(is.na(liv_all$values), NA, (liv_all$values/liv_all$ref_values))
	
	
		
		am <- NULL
		for (aa in unique(liv_all$scenario_year)){
		for (ab in unique(liv_all$rgn_id)){
			
			al <- which(liv_all$scenario_year == aa & liv_all$rgn_id == ab)
		...
		if (length(ald > 0)){
			...
			if (nrow(ald) > 1){
				...
				liv_score <- mean(ald$status_score, na.rm = T)
				
				aldk <- data.frame(rgn_id = ab, year = aa, status_score = liv_score)
		...
		scores <- rbind(status, trend)%>%mutate(goal = "LIV")%>%data.frame()
		scores <- scores%>%select(goal, dimension, region_id, score)
...
ECO <- function(layers){
	...
	#number marine production
	#########################
	marine_prod_lyrs <- c("eco_marine_production")
	marine_prod <- SelectData2(marine_prod_lyrs)
	
		#change colnames
		#################
		names(marine_prod) <- c("layer_name", "scenario_year", "data_year", "rgn_id", "values")
		
			
	
	#determine reference point for each layer
	#########################################
	ref <- marine_prod %>% group_by(layer_name, rgn_id)%>%summarize(ref_values = max(values, na.rm = T))%>%ungroup()
	
				
	
	#merge layers with reference points
	###############################	
	eco_all <- merge(marine_prod, ref, by = c("layer_name", "rgn_id"))
	
	#calculate status score for each year-region
	############################################
	eco_all$status_score <- ifelse(is.na(eco_all$values), NA, (eco_all$values/eco_all$ref_values))
	
		
	am <- eco_all%>%select(rgn_id, scenario_year, status_score)%>%rename(rgn_id = rgn_id, year = scenario_year, status_score = status_score)%>%arrange(rgn_id, year)
		...
		scores <- rbind(status, trend)%>%mutate(goal = "ECO")%>%data.frame()
		scores <- scores%>%select(goal, dimension, region_id, score)
