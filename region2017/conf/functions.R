...
CS <- function(layers){
	...
	########################################################
		...
		###########################
		#Teja I added a group by scenario_year for the total extent calculation so that the annual extent is the reference point
	  #prior, you were summing all extents for all years and artifically inflating habitat extent
	  total_extent <- extent%>%group_by(habitat, scenario_year)%>%summarize(total_extent = sum(km2, na.rm = T))%>%ungroup()
		...
		#Teja, note that when you merge these data sets you lose 2016 health data
