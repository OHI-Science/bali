...
FIS <- function(layers){
  ...
  r.trend <- r.trend %>%
    group_by(rgn_id) %>%
    do(mdl=lm(status_score~year, data=.)) %>%
  ...
  am <- am %>%
    group_by(rgn_id) %>%
    summarize(score = round(mean(status_score, na.rm = T), 2)*100) %>% ## Teja: status score should be the most recent year, not the mean
    ungroup()
  ...
  trend <- r.trend %>%
    ...
    rename(region_id = rgn_id, score = trend)
...
MAR <- function(layers){
  ...
  w <- status %>%
    group_by(rgn_id) %>%
    summarize(total_ysmi = sum(y_smi, na.rm = T)) %>%
    ungroup()
 ...
 ## Teja, I rewrote the above code because I wanted to understand what was happening with summarize, merge, new variable $w ... this is an alternative way to write it (and avoids merging `status` and `w` with quite different dimensions:: dim(status) is 80 x 6 and dim(w) is 8 x 2

  # status2 <- status %>%
  #   mutate(y_smi = ton * msi) %>% ## ## Teja just a suggestion for documentation clarity: should this and following variables be "_msi" instead of "_smi"
  #   group_by(rgn_id) %>%
  #   mutate(total_ysmi = sum(y_smi, na.rm = T)) %>% ## mutate() doesn't collapse like summary() does
  #   ungroup() %>%
  #   mutate(w = y_smi / total_ysmi) # dim: 80x6
  #
  # skimr::skim(status2)
  ...
  ref <- status %>%
    group_by(rgn_id) %>%
    summarize(max_prod = max(ton, na.rm = T)) %>% ## Teja I'm not sure why this would return so many -Inf but it is worth checking, it might contribute to the low final scores
    ungroup()
  ...
  status$x <- status$y_smi/(status$ref * potential_extent) # Teja does this calculation reflect what you want it to? Results in very low scores
  ...
  ## Julie rewrite above code for troubleshooting:
  # ref2 <- status2 %>%
  #   group_by(rgn_id) %>%
  #   mutate(max_prod = max(ton, na.rm = T)) %>%
  #   ungroup() %>%
  #   mutate(ref = max_prod / km2,
  #          x   = y_smi / (ref * potential_extent),
  #          score = round(w * x , 4))
  #
  # skimr::skim(ref2)
...
AO <- function(layers){
  ...
  am <- am %>%
    group_by(rgn_id) %>%
    summarize(score = round(mean(status_score, na.rm = T), 4)*100) %>% ## Teja: status score should be the most recent year, not the mean
    ungroup()
...
CS <- function(layers){
  ...
  ## Teja: here again with the max() function there are many -Inf values that I would review and make the following is working as you expect
  ...
  am <- am %>%
    group_by(rgn_id) %>%
    summarize(score = round(sum(status_score, na.rm = T), 4)*100) %>% ## Teja: status score should be the most recent year, not the sum
    ungroup()
...
CP <- function(layers){
  ...
  #status CP
  ###########
  am <- am%>%group_by(rgn_id)%>%summarize(score = round(sum(status_score, na.rm = T), 4)*100)%>%ungroup() ## Teja: status score should be the most recent year, not the sum
