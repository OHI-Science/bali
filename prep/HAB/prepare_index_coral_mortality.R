###############################################################
#prepare health of coral reef derived from index mortality data
###############################################################
...
setwd(dir_scripts)
	...
	dat_all <- dat_all%>%select(rgn_id, year, index_mortality)%>%arrange(rgn_id, year)
		...
		# save data layer
		#################
		write_csv(dat_all, paste(directory, "prep/", toupper(goal), "/", goal, "_", filename, "_", region, year, ".csv", sep = ""))
		write_csv(dat_all, paste(directory, "region2017/layers/", goal, "_", filename, "_", region, year, ".csv", sep = ""))
