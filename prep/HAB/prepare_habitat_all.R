...
variables <- c("coral", "mangrove", "seagrass")
	...
	for (i in 1:length(variables)){
			...
			if (variables[i] == "coral"){
				...
				dat <- readr::read_delim(file = paste("hab_", variables[i], "_reef.csv", sep = ""), delim = ";")
				
			}else{
				
				
				dat <- readr::read_delim(file = paste("hab_", variables[i], ".csv", sep = ""), delim = ";")
		...
		## save data layer
		write_csv(dat, paste("prep/HAB/hab_", variables[i], "_complete.csv", sep = ""))
		write_csv(dat, paste("region2017/layers/hab_", variables[i], "_complete.csv", sep = ""))
