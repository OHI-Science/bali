...
filename <- "aquaculture_production"
...
setwd(dir_scripts)
	...
	#equally divided into eight sub-region
	######################################
	dat$ton <- dat$ton/8
	
	#add new variable that will be used as rgn_id
	#############################################	
	dat <- dat%>%group_by(years)%>%mutate(rg = seq(1,8,1))%>%select(rg, years, ton)%>%
			rename(rgn_id = rg, year = years, ton = ton)
			
			
	#for trend calculation, data are divided with 100 000
	#######################################################
	dat$ton_divided <- ifelse(is.na(dat$ton), NA, dat$ton/100000)
			
			
	#calculate trend
	################
	r.trend <- dat%>%filter(year >= min(dat$year))%>%
				filter(!is.na(ton_divided))%>%
				group_by(rgn_id)%>%arrange(year)%>%
				top_n(5, year)%>%ungroup()
				
	r.trend <- r.trend%>%group_by(rgn_id)%>%do(mdl=lm(ton_divided~year, data=.))%>%
	...
	dat_all <- dat_all %>% arrange(rgn_id, year)
		...
		#save status data
		#################
		dat_stat <- dat_all%>%select(rgn_id, year, ton)
