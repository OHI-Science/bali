#####################################
#prepare mariculture production layer
#####################################
...
filename <- "mariculture_production"
...
setwd(dir_scripts)
	...
	dat <- dat%>%select(rgn_id, year, values)%>%rename(rgn_id = rgn_id, year = year, ton = values)%>%arrange(rgn_id, year)
